...
## AnyKernel setup
# EDIFY properties
kernel.string=ZER0 Byte Kernel By BryanByteZ At XDA-Devs
do.devicecheck=0
do.initd=0
do.modules=0
do.cleanup=1
...
write_boot() {
  cd $split_img;
  cmdline="androidboot.bootdevice=msm_sdcc.1 androidboot.hardware=qcom vmalloc=400M utags.blkdev=/dev/block/platform/msm_sdcc.1/by-name/utags androidboot.selinux=permissive zcache.enabled=1  zcache.compressor=lz4";
  board=`cat *-board`;
  base=`cat *-base`;
...
# adapt the idea of blechdose and modified by me (hurtsky) for anykernel script usage
# rest in peace mpdecision - hurtsky
#
